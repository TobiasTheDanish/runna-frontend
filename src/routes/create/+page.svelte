		...
		<CardContent>
			<form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-6">
				<div class="space-y-2">
					...
					<Popover bind:open>
						...
						<PopoverContent class="w-auto p-0" align="start">
							<Calendar bind:value={selectedDate} type="single" />
						</PopoverContent>
					</Popover>
				</div>
				...
				<div class="space-y-2">
					...
					<Input type="number" id="distance" bind:value={distance} step="0.01" min="0" required placeholder="e.g. 5.0" />
				</div>
				...
				<div class="space-y-2">
					...
					<div class="flex items-center gap-2">
						<div class="relative flex-1">
							...
							/>
							<span class="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">min</span>
						</div>
					...
					<p class="text-[0.8rem] text-muted-foreground">
						Enter your run duration in MM:SS format
					</p>
				</div>

				<div class="space-y-2">
					<Label for="notes">Notes (optional)</Label>
					<Textarea id="notes" bind:value={notes} rows={4} placeholder="How did the run feel?" />
				</div>
				...
				<div class="flex justify-end gap-3 pt-4">
					<Button type="button" variant="secondary" onclick={() => goto('/')} disabled={loading}>
						Cancel
					</Button>
